/*
 ...
 suit (thanks to someone who helpfully wrote its box IDs on the wall of the warehouse
 in the middle of the night). Unfortunately, anomalies are still affecting them - nobody
...
module SquareHitCmp =
  Belt.Id.MakeComparable({
    type t = (int, int);
    let cmp = ((aX, aY), (bX, bY)) => {
      switch (Pervasives.compare(aX, bX)) {
      | 0 => Pervasives.compare(aY, bY)
      | c => c
      };
    };
  });
...
let addSquaresFromClaim = (aClaim, claimMap) => {
  ...
  for (x in edgeX to edgeX + xSize - 1) {
    for (y in edgeY to edgeY + ySize - 1) {
      ...
      claimMap->Belt.MutableMap.set(
        (x, y),
        switch (existingCountForSquare) {
        | Some(count) => count + 1
        | None => 1
        },
      );
...
let calculateClaimsPerLocation = claims => {
  let claimMap = Belt.MutableMap.make(~id=(module SquareHitCmp));
  claims->Belt.List.forEach(aClaim => aClaim->addSquaresFromClaim(claimMap));
  claimMap;
};
